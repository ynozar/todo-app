...
public class DbContextHealthCheck<TContext> : IHealthCheck where TContext : DbContext
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _ = await _context.Set<ToDoItem>().Take(1).ToListAsync(cancellationToken);
            _ = await _context.Set<Group>().Take(1).ToListAsync(cancellationToken);
            _ = await _context.Set<User>().Take(1).ToListAsync(cancellationToken);
